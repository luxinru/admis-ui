  ...
  <div>
    <el-dialog :title="title" v-model="open" width="50vw" top="5vh" append-to-body>
      <div class="app-container">
        ...
        <el-table @current-change="getRowData" highlight-current-row v-loading="loading" :data="dataList"
                  @row-dblclick="toDownLevel">
...
function confirm() {
  ...
  //如果选的是明细级数据，或者允许选择非明细数据，都将返回选中的数据
  if (rowData.markTail === '1' || props.allowChooseParent) {
    // 明细数据,传回父组件
    emit('handleDepartCard', rowData);
    close();
  } else {
