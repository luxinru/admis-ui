<template>
  <div class="app-main-content">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="success" plain icon="View" :disabled="single" @click="handleDetail" >明细</el-button>
      </el-col>
      <el-col :span="1.5" >
        <el-button type="success" plain icon="Position" :disabled="commitFlag" @click="handleAudit"
                   v-hasPermi="['invoice:assetsaudit:submit']">审批</el-button>
      </el-col>
...
<script setup name="FinancialInvoice">
  ...
  import { listInvoice, commitFinancialInvoice, auditInvoice } from "@/api/value/add/assetsaudit/invoice/invoice";
  import auditPage from "@/views/value/add/assetsaudit/components/audit/index";
  ...
  const data = reactive({
    ...
    queryParams: {
      ...
      pageSize: 10,
      invoiceNumber: null,
      invoiceStatus: Base64.encode(" and invoice_status in ('21')"),
      projectCode: null,
      projectName: null
    },
    rules: {
  ...
  function handleDetail() {
    const invoiceNumber = invoiceNumbers.value[0];
    router.push({
      path: "/value/add/assetsaudit/detail",
      query: {
  ...
  function handleApprovalData(flag, reason) {
    ...
    if(flag){
      ...
      auditInvoice(obj).then(response => {
