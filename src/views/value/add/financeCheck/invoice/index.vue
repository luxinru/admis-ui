<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="warning" plain icon="Download" @click="handleExport"
          v-hasPermi="['invoice:financeCheck:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="checkList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
        <el-table-column label="单据编号" align="center" width="200">
          <template #default="scope">
            <router-link :to="{
                        path:'/value/add/financeCheck/detail',
                        query:{
                          invoiceNumber:scope.row.invoiceNumber,
                          invoiceType:scope.row.invoiceType
                        }
                  }"  class="link-type">
      ...
      <el-table-column label="汇总预备费" align="center" prop="prepareCostSum" />
      <el-table-column label="汇总建设期贷款利息" align="center" prop="loanInterestSum" />
...
// 引入明细操作方法
import { listDetail, getDetail } from "@/api/value/add/financeCheck/detail/detail";
...
function handleSelectionChange(selection) {
  ...
  invoiceTypes.value = selection.map(item => item.invoiceType);
}
...
function handleExport() {
  proxy.download('invoice/financeCheck/export', {
    ...queryParams.value
...
function handleDetail() {
  router.push({
    ...
    query: {
      ...
      invoiceType: invoiceTypes.value[0]
    },
  });
}
