    ...
    <div class="div-editor-container-grey">
      <div class="table-box">
        ...
        <el-table v-loading="houseRefineLoading" :data="houseRefineList"
                  @selection-change="houseRefineHandleSelectionChange" :height="height" :row-class-name="tableRowClassName">
          <el-table-column type="selection" width="55" align="center"/>
          <el-table-column label="资产编码" align="center" prop="assetsCode" width="270"/>
...
<script setup>
import {
  cancelHouseRefineAssetsData, cancelAssetsData,
  ...
  getHouseRefineDetail, auditAndQueryGetHouseRefineGetList
...
function assetsHandleSelectionChange(selection) {
  ...
  const keyCodesTepm = selection.map(item => item.keyCode);
  ...
  if (selection.length == 0 || selection.length > 1) {
    ...
    // 查询房屋卡片列表数据，入参
    houseRefineQueryParams.value.pkKeyCode = (keyCodesTepm[0] == "" ? "-1" : keyCodesTepm[0]);
    houseRefineGetList();
  }
}
...
function houseRefineGetList() {
  houseRefineLoading.value = true;
  auditAndQueryGetHouseRefineGetList(houseRefineQueryParams.value).then(response => {
...
function houseRefineHandleSelectionChange(selection) {
  ...
  houseRefineKeyCodes.value = selection.map(item => item.keyCode);
}
...
function cancelHouseRefineAssets() {
  ...
  const obj = {
    ...
    keyCodes: houseRefineKeyCodes.value,
  };
  proxy.$modal.confirm('是否确认撤销选中的数据项？').then(function () {
